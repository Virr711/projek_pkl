...
if (!empty($search)) {
    ...
    $search_fields = ["kode_plat", "nama", "lokasi_ruas", "merk", "penanggung_jawab", "no_chasis", "no_mesin", "kondisi", "catatan"];
    $search_clauses = [];
...
<div class="container-fluid p-0">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    ...
                    <li class="breadcrumb-item active text-white" aria-current="page">Submenu Data Unit</li>
                </ol>
            </nav>
            <h4 class="fw-bold mb-0 text-white"><i class="fa-solid fa-truck-monster text-info me-2"></i> Laporan Data Unit (Inventarisasi Kendaraan & Peralatan Aset)</h4>
        </div>
        <div class="d-flex gap-2">
            ...
            <?php if ($current_role === 'admin' || $current_role === 'pimpinan'): ?>
                <a href="export_docx.php?tipe=data_unit&<?= htmlspecialchars($_SERVER['QUERY_STRING'] ?? '') ?>" class="btn btn-primary rounded-pill px-4 shadow" style="background-color: #0d9488 !important; border: none;">
    ...
    <!-- FILTER BAR INTERAKTIF DATA UNIT & SEARCH -->
        ...
        <form method="GET" action="" class="row g-2 align-items-center">
            <div class="col-md-3">
                ...
                <div class="input-group">
                    <input type="text" name="search" class="form-control bg-dark text-white border-secondary" placeholder="Cari Nopol, Nama, Merk..." value="<?= htmlspecialchars($search) ?>">
                    <button type="submit" class="btn btn-outline-secondary text-white"><i class="fa-solid fa-search"></i></button>
                </div>
            </div>
            <div class="col-md-3">
                <label class="form-label fw-bold text-white-50 small mb-1"><i class="fa-solid fa-truck me-1"></i> Filter Klasifikasi Unit:</label>
                <select name="jenis" class="form-select bg-dark text-white border-secondary" onchange="this.form.submit()">
                    <option value="">-- Semua Jenis (Semua Unit) --</option>
            ...
            <div class="col-md-1 d-flex align-items-end" style="height: 60px;">
                <?php if (!empty($jenis_filter) || !empty($kondisi_filter) || !empty($ruas_filter) || !empty($search)): ?>
                    <a href="laporan_unit.php" class="btn btn-outline-secondary w-100 rounded-pill mb-1 p-1 text-center" title="Reset Filter"><i class="fa-solid fa-rotate-left"></i></a>
                <?php endif; ?>
            </div>
        </form>
    </div>
    ...
    <div class="card-custom p-4" id="printableArea">
        <div class="text-center mb-4 pb-3 border-bottom border-secondary">
            ...
            <h6 class="fw-bold text-info mb-1">DOKUMEN RESMI LAPORAN INVENTARISASI ASET DATA UNIT & PERALATAN</h6>
            <p class="small text-white-50 mb-0">Tahun Anggaran 2026 &bull; Tanggal Cetak: <?= format_tgl_indo(date('Y-m-d')) ?></p>
        </div>

        <div class="d-flex justify-content-between align-items-center mb-3">
            <h6 class="fw-bold text-white mb-0">Data Unit Peralatan dan Kendaraan (Aset Logistik BPJ Tegal):</h6>
            ...
            <table class="table table-hover align-middle small">
                <thead>
                    <tr>
                        ...
                        <th class="text-nowrap text-end no-print">Riwayat Servis</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($data_report)): ?>
                        <tr><td colspan="10" class="text-center text-muted py-4">Tidak ada data unit aset sesuai dengan kriteria filter & pencarian.</td></tr>
                    <?php else: ?>
                        <?php foreach ($data_report as $idx => $row): ?>
                            <tr>
                                ...
                                <td class="text-end text-nowrap no-print">
                                    <a href="laporan_servis.php?id_kendaraan=<?= $row['id'] ?>" class="btn btn-xs btn-outline-info rounded-pill px-2.5 py-1" title="Cetak Laporan Riwayat Servis Unit Ini">
                                        <i class="fa-solid fa-file-contract me-1"></i> Servis
                                    </a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
